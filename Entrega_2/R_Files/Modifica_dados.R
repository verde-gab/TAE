list.files() #lista os arquivos dentro da pasta
getwd() #mostra o endereção da pasta 
# caso precise mudar de pasta e exibir o local do arquivo 
#setwd("/home/giovanna/Downloads/Porto_Santos/Porto_Santos") #Caso precise mudar a pasta 
getwd()
dados <- read.csv("exportacao_cargas.csv") #lê o arquivo csv e cria um dataframe chamado dados no script em R
# ── Contadores globais ────────────────────────────────────────────
...
limpar_texto <- function(x, col) {
  ...
  # Verifica se o nome da coluna é ano ou mes (minúsculo, pois
  # limpar_colunas() já padronizou os nomes para lowercase antes
  # de limpar_dados() ser chamada).
  ...
  # Verifica se o nome da coluna é ano_mes (minúsculo, pelo mesmo
  # motivo acima — limpar_colunas() já converteu para lowercase).
...
str(dados)
# FUNÇÃO: tratar_zeros_teu()
# Objetivo: substituir zeros em total_teu por NA quando a natureza
# da carga não for conteinerizada — nesses casos o TEU é um ausente
# estrutural (o conceito não se aplica), não um valor real igual a zero.
# Recebe o dataframe completo e retorna o dataframe corrigido.
tratar_zeros_teu <- function(dados) {
  
  # Exibe contagens antes da substituição para comparação posterior.
  cat("ANTES DA SUBSTITUICAO\n")
  cat(rep("-", 40), "\n", sep = "")
  # sum() com condição lógica conta quantas linhas atendem ao critério.
  cat("Zeros em total_teu:                  ", sum(dados$total_teu == 0, na.rm = TRUE), "\n")
  cat("Registros nao conteinerizados:       ", sum(dados$natureza_carga != "carga_conteinerizada", na.rm = TRUE), "\n")
  cat("Zeros em total_teu E nao container:  ", sum(dados$total_teu == 0 & dados$natureza_carga != "carga_conteinerizada", na.rm = TRUE), "\n")
  cat(rep("-", 40), "\n", sep = "")
  
  # ifelse() avalia uma condição linha a linha e retorna valores
  # diferentes dependendo se a condição é TRUE ou FALSE.
  # Condição: total_teu é zero E a natureza da carga NÃO é conteinerizada.
  # TRUE  → substitui por NA (ausente estrutural — TEU não se aplica).
  # FALSE → mantém o valor original (zero legítimo ou valor positivo).
  dados$total_teu <- ifelse(
    dados$total_teu == 0 & dados$natureza_carga != "carga_conteinerizada",
    NA,               # ausente estrutural — TEU não se aplica
    dados$total_teu   # mantém o valor original
  )
  
  # Exibe contagens depois da substituição para confirmar o resultado.
  cat("DEPOIS DA SUBSTITUICAO\n")
  cat(rep("-", 40), "\n", sep = "")
  cat("Zeros restantes em total_teu:        ", sum(dados$total_teu == 0, na.rm = TRUE), "\n")
  cat("NAs em total_teu:                    ", sum(is.na(dados$total_teu)), "\n")
  cat(rep("-", 40), "\n", sep = "")
  
  # Retorna o dataframe com a coluna total_teu corrigida.
  return(dados)
}

# ── Execução ───────────────────────────────────────────────────────
# Aplica a substituição e sobrescreve o dataframe com o resultado.
dados <- tratar_zeros_teu(dados)
str(dados)
